...
function EditCourse(props) {
  ...
  useEffect(() => {
    axios.get(`/api/courses/${id}`).then((res) => console.log(res));
  }, [id]);
  ...
  return (
    <div className="admin-add-course-container">
      <div className="add-course-step-one-container">
        {requestSent && (
          <div className="add-course-step-one-progress-bar-container">
            ...
            <p className="add-course-step-one-progress-bar-text">
              در حال ارسال اطلاعات به سرور می باشیم ، بسته به حجم فایل های
              آپلودی و سرعت اینترنت شما این پروسه ممکن است چند دقیقه به طول
              بیانجامد
              {<br />}
        ...
        {!requestSent && (
          <>
            <h1 className="add-course-step-one__title">ویرایش تنظیمات دوره</h1>
